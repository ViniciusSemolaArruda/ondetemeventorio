...
export function resolveBaseURL(): string {
  ...
  const fromExtra = Constants.expoConfig?.extra?.API_BASE_URL as
    | string
    | undefined
...
const coreHelpers = {
  ...
  likeEvent: async (id: string) =>
    (
      await api.post<{ liked: boolean; count: number }>(
        `/api/events/${id}/like`,
      )
    ).data,
  ...
  me: async () =>
    (
      await api.get<{ user: ApiUser | null; accessToken?: string }>(
        "/api/auth/me",
      )
    ).data,
  ...
  mobileLogin: async (payload: { id_token: string }) =>
    (
      await api.post<{ user: ApiUser; accessToken: string }>(
        "/api/mobile-login",
        payload,
      )
    ).data,
  ...
  saveUserPreferences: async (preferences: string[]) =>
    (
      await api.post<{ message: string }>("/api/users/preferences", {
        preferences,
      })
    ).data,
...
async function sendChatMessage(text: string) {
  const res = await api.post<{ ok: true; message: ChatMessage }>(
    "/api/chat/send",
    { text },
  )
  return res.data
...
async function reopenConversation(conversationId: string) {
  const res = await api.post<{ ok: true }>(
    `/api/chat/${encodeURIComponent(conversationId)}/reopen`,
  )
  return res.data
}
async function closeConversation(conversationId: string) {
  const res = await api.post<{ ok: true }>(
    `/api/chat/${encodeURIComponent(conversationId)}/close`,
  )
  return res.data
}
async function deleteConversation(conversationId: string) {
  const res = await api.delete<{ ok: true }>(
    `/api/chat/${encodeURIComponent(conversationId)}/delete`,
  )
  return res.data
}
...
async function replyToConversation(p: {
  conversationId: string
  text: string
}) {
  const res = await api.post<{ ok: true; message: ChatMessage }>(
